...
from ultralytics.nn.modules.SR_model import SR_model
import torch
...
class BaseModel(torch.nn.Module):
    ...
    def loss(self, batch,epoch, preds=None,output_sr=None):
        """
        Compute loss.
        ...
        if (preds == None):
            preds, output_sr = self.forward(batch["img"])
            return self.criterion(preds, batch, output_sr, isTrain=self.training, epoch=epoch)
        else:
            return self.criterion(preds, batch, output_sr, isTrain=self.training, epoch=epoch)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True, factor=4):
        """
        ...
        self.model_up = SR_model(3, self.yaml['c1'], self.yaml['c2'],
                                factor=factor)  # .cuda() #'if the size is m:192,768 l:256,1024 x:320 1280
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m,  (Detect, Segment, Pose)):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            s = 256  # 2x min stride
            ...
            def _forward(x):
                ...
                if self.end2end:
                    uuuu = self.forward(x)
                    # return self.forward(x)["one2many"]
                    return uuuu[0]["one2many"]
                if isinstance(m, (Segment, Pose, OBB)):
                    uuuu = self.forward(x)[0][0]
